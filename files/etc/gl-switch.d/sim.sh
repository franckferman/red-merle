#!/bin/sh
action=$1
logger -p notice -t red-merle-toggle  "Called... ${action}"
...
if [ "$action" = "on" ];then
    mcu_send_message "Red Merle ${action}"
    echo "on" > /tmp/sim_change_switch
    flock -n /tmp/red-merle-switch.lock logger -p notice -t red-merle-toggle "Running Stage 1" ||  logger -p notice -t red-merle-toggle  "Lockfile busy"
    ...
    if [ -f /tmp/red-merle-stage1 ]; then
        flock -n /tmp/red-merle-switch.lock  ||  logger -p notice -t red-merle-toggle  "Lockfile busy" &
        flock -n /tmp/red-merle-switch.lock  timeout 90  /usr/bin/red-merle-switch-stage2
    else
        logger -p notice -t red-merle-toggle  "No Stage 1; Toggling Off"
    fi
...
logger -p notice -t red-merle-toggle  "Finished Switch $action"
sleep 1
